...
# run the command
res1=$(date +%s.%N)
$toolPath -i $input -o $outdir -k $kmer >> $logfile 2>&1
res2=$(date +%s.%N)
...
basename1=$(basename $input)

cat $outdir/${basename1}.corrected | gzip > $outdir/${toolName}_$(basename ${input%.*})_${kmer}.corrected.fastq.gz
